...
class MyEngine(Engine):
    ...
    def train(engine, mini_batch): # tuple in mini-batch
        ...
        engine.model.train() # Because we assign model as class variable, we can easily access to fit.
    ...
    def check_best(engine):
        ...
        if loss <= engine.best_loss: # If current epoch returns the lower validation loss,
            ...
            engine.best_model = deepcopy(engine.model.state_dict()) # Update the best model weights.
...
class Trainer():
    ...
    def train(
        ...
        train_engine = MyEngine(
            ...
            model, crit, opimizer, self.config
        )
